...
if(isset($_POST['act']) && ($_POST['act']=='make')){
    ...
    if($txtwidth<=0 || $$txtwidth>=500){
        $txtwidth = 100;
    }
    
    $txtheight = intval($_POST['txtheight']);
    if($txtheight<=0 || $$txtheight>=500){
        $txtheight = 100;
    }
...
$tpl = <<<EOF
<script type="text/javascript" src="http://{$rand}.{$domain}/abs/abs.php?type=0&num={$num}&uid={$uid}&width={$txtwidth}&height={$txtheight}&cid=0&color=0&pid={$shopids}"></script>
EOF;
        	  ...
        	  <h3 class="zaixiancz-icon" style="background-position: 0 -300px;">推广工具</h3>
                <div class="vip-content-r-zijinsousuo" style="height: 1150px; min-width:1000px;">
                    ...
                    <div class="chongzhitaocan">
                        ...
                        <div>
                        <span>图片宽度</span>          
                        <input type="text" id="txtwidth" name="txtwidth" value="100" />
                        </div>
                        
                        <div>
                        <span>图片高度</span>          
                        <input type="text" id="txtheight" name="txtheight" value="100" />
                        </div>
...
$(function(){
    $("#make").click(function(){
        var num,shopids,txtwidth,txtheight;
        num = $("#num").val();
        ...
        $.ajax({
           cache: false,
  		   ...
  		   type: 'post',
  		   url: 'cps_2.php',
  		   data:{shopids:shopids,txtwidth:txtwidth,txtheight:txtheight,num:num,act:'make'},
