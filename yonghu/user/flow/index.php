...
//全部订单
$sql = "SELECT count(*) as num,date_format(addtime,'{$format}') as ymd FROM `wx_guest` {$where} group by ymd";
...
//pv
$pv = array();
$sql = "SELECT count(*) as num,date_format(addtime,'{$format}') as ymd FROM `wx_tongji` {$where_pv} group by ymd";
$result = mysql_query($sql);
while(($row = mysql_fetch_assoc($result))!==false){
	$pv[$row[ymd]] = $row['num'];
}
