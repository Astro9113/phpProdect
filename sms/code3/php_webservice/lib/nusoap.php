	...
	*/
	function schemaStartElement($parser, $name, $attrs) {
		...
		switch($name){
			...
			case 'require':
			    if (isset($attrs['schemaLocation'])) {
					$this->xdebug('require into namespace ' . $this->schemaTargetNamespace . ' from ' . $attrs['schemaLocation']);
                    $this->imports[$this->schemaTargetNamespace][] = array('location' => $attrs['schemaLocation'], 'loaded' => false);
				} else {
					$this->xdebug('ignoring invalid XML Schema construct: require without schemaLocation attribute');
				}
			break;
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
			$this->setError('The PHP cURL Extension is required for HTTPS or NLTM.  You will need to re-build or update your PHP to require cURL or change php.ini to load the PHP cURL extension.');
			return false;
		}
