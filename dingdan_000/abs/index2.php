<?php 
    ...
    $colors = array('#0082a6','blue','#000','green','#ff00ff','#0082a6');
    $sql  = "select shopid,file_index,title from wx_shop_addon where hide=0 and shopid in ({$pid}) order by rand() limit $num ";
    ...
    while ($row = mysql_fetch_assoc($result)){
        ...
        $_color = $color?$colors[array_rand($colors)]:'#000'; 
        
        $infos[] = array('title'=>$_title,'link'=>$link,'color'=>$_color);
    }
                    ...
                    <div style="width: 99%; float: left;">
                        <ul>
                                        ...
                                        foreach ($infos as $info){
                                            $tpl = '<li class="article_tj" style="line-height:25px;"><a target="_parent" href="%s" style="color: %s">
                                      <span style="line-height: 20px">•%s</span></a>
                                   
                                        </li>';
                                            $html = sprintf($tpl,$info['link'],$info['color'],$info['title']);
                                            echo $html;
                                        }
                                        ?>
