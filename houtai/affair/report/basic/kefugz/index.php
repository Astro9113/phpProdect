        ...
        <div class="tjindex filter">
			<div class="cxform2 mtop12" style="width: 800px;">
				...
				<form id="search" action="">
					查询时间：<input ph="开始时间" type="text" id="time1" name="time1"
						value="<?php echo $time1;?>" size="11"
						onClick="WdatePicker({startDate:'%y-%M-%d 00:00:00',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> - <input ph="结束时间"
						type="text" id="time2" name="time2"
						value="<?php echo $time2;?>" size="11"
						onClick="WdatePicker({startDate:'%y-%M-%d 23:59:59',dateFmt:'yyyy-MM-dd HH:mm:ss'})" /> &nbsp;&nbsp; 客服：<select
						name="kfid">
						<option value="0">客服订单查询</option>
...
if ($time1 && $time2 && $kfid) {
    ...
    foreach ($xun as $i => $class) {
        ...
        $time1 . "&time2=" . $time2 . "&aa=" . $class . "&kfid=" . $kfid . "'></iframe>
              </div>";
    ...
    $sql = "select g.shopid,g.skuid,s.shopsku1,s.shopsku2,s.shopsku3,s.shopsku4,s.shopsku5,s.shopsku6 from wx_guest g left join wx_shop s on g.shopid = s.id where g.guestkfid = '{$kfid}' and g.addtime>='$time3' and g.addtime<='$time4' order by g.id";
    ...
    ?>
<div class="middtj">
总数：<d class="nowstat"><?php echo $num;?></d>
&nbsp;&nbsp;&nbsp; 流水：
