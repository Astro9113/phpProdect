...
<div class="menu">
 ...
 <dl class="cur">
    ...
    <dd class="last"><a href="/vukefu/stat/lbs/">●　联不上订单</a></dd>
  ...
  <dl class="expand cur">
    ...
    <dd class="last"><a class="now" href="/vukefu/stat/important//lbs/">●　联不上订单</a></dd>
 </dl> 
...
<?php
	...
	foreach($arr as $k=>$v){
		$sql = "select count(*) as num from wx_guest g left join wx_lbsguest lbs on g.id = lbs.guestid where g.guestkfid = '{$kefid}' and g.gueststate=11 and lbs.stateid = $k   and g.userid in(select id as userid from wx_user where isimportant = 1) ";
		$result = mysql_query($sql);
...
<div class="cb"></div>
 ...
 <div class="uc">
  ...
  if($class==''){ $class='0';}
    if($class=='0'){	  
$sql = "select g.*,lbs.guestid,lbs.stateid from wx_guest g left join wx_lbsguest lbs on g.id = lbs.guestid where  g.addtime <= '{$lbs_start_data}' and g.guestkfid = '{$kefid}' and g.gueststate=11  and g.userid in(select id as userid from wx_user where isimportant = 1) group by g.id order by g.id desc;";
  }else{
 $sql = "select g.*,lbs.guestid,lbs.stateid from wx_guest g left join wx_lbsguest lbs on g.id = lbs.guestid where  g.addtime <= '{$lbs_start_data}' and g.guestkfid = '{$kefid}' and g.gueststate=11 and lbs.stateid='$class' and g.userid in(select id as userid from wx_user where isimportant = 1) group by g.id order by g.id desc;";
  }
    $sql=mysql_query($sql);
