...
<?php require KEFU_PATH . 'menu.php';?>
    <div id="main" class="clearfix">
        <div class="filter widthb110">
            <?php
                $num1 = get_num_today($loginid, 2,1);
                $num2 = get_num_today($loginid, 11,1);
                $num3 = get_num_today($loginid, 9,1);
            ?>
            <div class="cxform2" style="width: 800px; line-height: 38px;">
                            重点客户 今日确认中：<?php echo $num1;?> ， 联不上：<?php echo $num2;?> ， 待发货：<?php echo $num3;?><br>
            </div>
...
foreach ($statearr as $state) {
    foreach ($timearr as $v) {
        ...
        $sql[] = "select * from wx_guest where guestkfid='$loginid' and gueststate = '{$state}' and userid in(select id as userid from wx_user where isimportant = 1) and addtime between '{$stime}' and '{$etime}'";
    }
}
...
$sql = "select * from wx_guest where addtime<='{$uptime}' and guestkfid='$kefid' and gueststate =11 and userid in(select id as userid from wx_user where isimportant = 1) order by id desc";
$infos = $mysql->query($sql);
