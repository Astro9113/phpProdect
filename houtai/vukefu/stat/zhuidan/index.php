...
//使用 不等号导致索引失效 有时间改写
$sql = "select g.*,gs.wuliustate,gs.guestid,gs.newwuliu,gs.gendan from wx_gueststate gs left join wx_guest g on g.id = gs.guestid where g.guestkfid = '{$kefid}' and gs.wuliustate not in (2,4,5) and g.userid not in(select id as userid from wx_user where isimportant = 1)  group by g.id order by g.id desc;";
