...
class MySQL{
	...
	private function connect(){
		...
		$this->conn = mysql_connect($server,$this->db_mysql_username,$this->db_mysql_password,true) or die('');
		mysql_select_db($this->db_mysql_database,$this->conn) or die('');
    ...
    public function query_assoc($sql,$pk){
        $ret = $this->query($sql);
        $ret2 = array();
        foreach ($ret as $row){
            $id = $row[$pk];
	...
	function count_table($table,$w = 1){
	    $sql = "select count(*) as num from {$table} where {$w}";
	    $ret = $this->find($sql);
	    $num = $ret['num'];
	    return intval($num);
	}
