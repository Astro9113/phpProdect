...
if(isset($_POST['up'])){
    ...
    */
    ...
    //计算对账表数量
    $sql = "select count(*) as num,stateid from wx_dzguest where guestid in (".join(',',array_keys($arr_guest)).") group by stateid";
    $infos = $mysql->query($sql);
    foreach ($infos as $info){
    ...
    foreach ($infos as $info){
        echo "{$info['id']}\t{$info['guestkuaidi']}\t{$info['stateid']}<br /><br />";
    }
    
    //系统结算 但没在在对账表是3，4的列出来
    $sql = "select g.id,g.guestkuaidi,d.stateid from wx_guest g left join wx_dzguest d on g.id = d.guestid where g.id in (".join(',',$arr_jiesuan).") and d.stateid IS NULL";
    $infos = $mysql->query($sql);
    echo '系统结算 但对账表不存在的: '.count($infos).' 个<br /><br />';
    ...
    //系统拒收 但没在对账表是1，2的列出来
    $sql = "select g.id,g.guestkuaidi,d.stateid from wx_guest g left join wx_dzguest d on g.id = d.guestid where g.id in (".join(',',$arr_jushou).") and  d.stateid IS NULL";
    $infos = $mysql->query($sql);
    echo '系统拒收 但对账表不存在的: '.count($infos).' 个<br>';
    foreach ($infos as $info){
        echo "{$info['id']}\t{$info['guestkuaidi']}<br /><br />";
    }
