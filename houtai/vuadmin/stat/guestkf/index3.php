...
$sql[] = "select id from wx_guest where addtime>='$time3' and addtime<='$time4' and guestkfid='0'  and is_sms = 3";
...
$sql[] = "select id from wx_guest where addtime>='$time3' and addtime<='$time4' and guestkfid='0' and timediff(now(),addtime) >= '00:04:00'  and is_sms = 3";
...
if($bgnumz > 0){
    ...
    foreach ($infos as $info){
        ...
        if(($info['gueststate']==9) && in_array($kfid, array(24,25,26,27,28,29,30,31))){
            $o++;
            echo '不分待发货,跳过<br />';
			continue;
        }
        
        //echo $info['gueststate'];
...
echo $zgdid;
$l++;
echo "<script>setTimeout(\"window.location.href='index3.php?l=".$l."'\",15000);</script>";
