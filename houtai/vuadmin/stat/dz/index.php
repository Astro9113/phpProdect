		...
		<div class="filter">
			<div class="cxform2" style="width: 400px;">
				<a href="index.php" style="color: #000;">全部已签收</a> <a
					...
					href="tui.php">退回签收</a> <a href="tui_yc.php">退回异常</a> <a
					href="dzsf/">周对账-顺丰</a> <a href="dzems/">周对账-EMS</a>
			</div>
			<div class="cxform2" style="width: 320px;">
				<form action="cha.php" method="get" style="float: right;">
					...
					<button type="submit">查询</button>
						...
						<div class="cxform2" style="width: 320px;">
				<form action="chadh.php" method="get" style="float: right;">
					<input type="text" name="dh">
					<button type="submit">查询单号</button>
				</form>
			</div>
			<div class="cb"></div>
		</div>
...
$sql = "select count(*) as num from wx_guest g left join wx_dzguest dg on g.id = dg.guestid where g.gueststate = 5 and g.id not in (select guestid as id from wx_dzguest)";
$ret = $mysql->find($sql);
$num = $ret['num'];


$sql = "select g.*,dg.stateid from wx_guest g left join wx_dzguest dg on g.id = dg.guestid where g.gueststate = 5 and g.id not in (select guestid as id from wx_dzguest)";
...
$sql .= " order by dg.stateid is null ,g.id  limit $offset,$page_size";
