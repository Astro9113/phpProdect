...
$sql = "select count(*) as num from wx_dzguest dg  left join wx_guest g on g.id = dg.guestid where g.gueststate = 5 and dg.stateid = 3";
$ret = $mysql->find($sql);
$num = $ret['num'];

$sql = "select g.*,dg.stateid from wx_dzguest dg  left join wx_guest g on g.id = dg.guestid where g.gueststate = 5 and dg.stateid = 3";
