...
<div class="uc mtop5">
	<form action="changestate.php" method="post" target="_blank">
		<input type="hidden" name="type" value="6ycto6zc">
...
<?php
$sql = "select count(*)  as num from wx_dzguest dg  left join wx_guest g on g.id = dg.guestid where dg.stateid = 2";
$ret = $mysql->find($sql);
$num = $ret['num'];
$sql = "select g.*,dg.stateid from wx_dzguest dg  left join wx_guest g on g.id = dg.guestid where dg.stateid = 2";
...
?>
        ...
        <p>
			<input type="submit" name="sub" value="改为退回签收">
		</p>
	</form>
